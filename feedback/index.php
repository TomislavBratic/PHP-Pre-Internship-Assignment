...
$nameErr = $emailErr = $bodyErr =$urlErr=$ratingErr= '';
...
if (isset($_POST['submit'])) {
 ...
 //Validate Rating
  if (empty($_POST['rating'])) {
    $ratingErr = 'Rating is required';//not used for insert
  } else {
  ...
  if (empty($nameErr) && empty($emailErr) && empty($bodyErr)&&empty($urlErr)) {
    ...
    <form method="POST" action="<?php echo htmlspecialchars(
      ...
      <div class="mb-3">
        ...
        <input type="number" class="form-control <?php echo !$ratingErr ?:
          'is-invalid'; ?>" id="rating" name="rating" placeholder="Enter your rating" min="1" max="5" value="<?php echo $rating; ?>">
      </div>
      <div class="mb-3">
        <label for="url" class="form-label">Video url</label>
        <input type="url" class="form-control <?php echo !$urlErr ?:
